...
contract Ownable {
    ...
    modifier onlyOwner {
        require(msg.sender == _owner, "Caller should be the owner of the contract");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "Address provided is not real");
        _owner = newOwner;
...
modifier whenNotPaused () {
    require(_paused == false, "This function can only be called when contract is not paused");
    _;
}

modifier paused () {
    require(_paused == true, "This function can only be called when contract is paused");
    _;
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public whenNotPaused{
        ...
        require(msg.sender == owner || isApprovedForAll(owner, msg.sender), "Caller is not authorized to approve addresses on this token");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        ...
        require(_tokenApprovals[tokenId] != address(0), "There is no approval for this token");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(_exists(tokenId) == false || to != address(0), "Token already exists or address provided is not valid");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == _tokenOwner[tokenId], "From address is not owner of the token");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "To address provided is not valid");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "Token doesn't exist with that ID");
...
contract DigitalPropertyToken is ERC721Metadata {
